...
def load_vectorstore():
    """Memuat koleksi Chroma DB secara langsung (From Scratch)"""
    if not VS_DIR.exists():
        raise FileNotFoundError(
            f"Vector store tidak ditemukan di '{VS_DIR}'.\n"
            "Jalankan dulu: python src/indexing.py"
        )
    # Memanggil fungsi embedding_function dari embeddings.py
    ef = get_embedding_function()

    client = chromadb.PersistentClient(path=str(VS_DIR))
    collection = client.get_collection(name="rag_collection", embedding_function=ef)
...
def retrieve_context(vectorstore, question: str, top_k: int = TOP_K) -> list:
    """
    LANGKAH 1 & 2: Query embedding + Similarity search (Chroma DB Native).
    """
...
- Jika pertanyaan menanyakan statistik, jumlah, dan data spesifikk prioritaskan pencarian pada data CSV sebelum melihat PDF
- Analisis dan jelaskan relasi antar entitas jika informasi tersebar di beberapa sumber
- Jika ada informasi yang saling bertentangan antar sumber, sebutkan perbedaan tersebut secara objektif
- Selalu akhiri jawaban dengan bagian "Solusi & Langkah Praktis:" yang diletakkan di akhir paragraf
...
def answer_question(question: str, vectorstore=None, top_k: int = TOP_K) -> dict:
    ...
    # Retrieve
    print(f"🔍 Mencari konteks relevan untuk: '{question}' (top_k={top_k})")
    
    # PERUBAHAN: Parameter top_k diteruskan ke fungsi retrieve_context
    ...
    # TODO: Pastikan LLM yang digunakan sudah sesuai (misal: Gemini)
